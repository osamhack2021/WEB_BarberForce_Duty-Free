...
router.post('/boards/:id/recommendation', fetchUser, async (req, res) => {
  try {
    ...
    //추천인 목록에 있는 걍우
    if (recommend_user) {
      await post.updateMany({ $set: { recommendation: -1 } }, { $pull: { recommend_user: req.params.id } });
    } else {
      await post.updateMany({ $set: { recommendation: 1 } }, { $push: { recommend_user: req.params.id } });
    }
    return res.json({});
  } catch (e) {
    console.error(`[${req.method}] ${req.path} - 에러!`, e);
